...
search_text = input("Entre the text for search")
...
## i will load page and get all url then click next and load new page and then repeat same process
url = []
count == 0
while True:
    ...
    try:
        while True:
            ...
            if height == new_height:
                break
            height=new_height
        products=driver.find_elements("css selector",".s-line-clamp-2")
        
        for u in products:
            url.append(u.get_attribute("href"))
        try:
            
            button = driver.find_element(By.XPATH, "//a[contains(text(), 'Next')]")
            button.click()
...
fileName = f"{search_text}_data.xlsx"
